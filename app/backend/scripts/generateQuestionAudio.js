...
const { voiceFor, textHash, synthesize } = require('../utils/edgeTts');
...
async function run() {
  ...
  console.log(`Found ${questions.length} active question(s). force=${force}`);
  ...
  for (const q of questions) {
    ...
    try {
      const voice = voiceFor(q.text);
      const audio = await synthesize(q.text, voice);
