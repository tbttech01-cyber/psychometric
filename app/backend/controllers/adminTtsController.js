...
exports.listStatus = async (req, res, next) => {
  try {
    const questions = await Question.find({ isActive: true }).select('_id text order').sort('order');
    const rows = await QuestionAudio.find({ questionId: { $in: questions.map((q) => q._id) } }).select('questionId textHash voice updatedAt');
    const byId = {};
    rows.forEach((r) => { byId[r.questionId.toString()] = r; });
    const data = questions.map((q) => {
      const r = byId[q._id.toString()];
      const status = !r ? 'none' : (r.textHash === textHash(q.text) ? 'ready' : 'stale');
      return { _id: q._id, order: q.order, text: q.text, status, voice: r ? r.voice : null };
    });
